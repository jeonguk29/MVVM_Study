...
final class ViewController: UIViewController {
    ...
    // MARK: - 뷰모델
    private let viewModel = LoginViewModel() // 뷰 모델 소유
    ...
    // ⭐️ UITextField만들때 바인딩기능있는텍스트필드로 생성함
    private lazy var emailTextField: UITextField = { // UP 캐스팅해서 저장할 뿐임
        var tf = 바인딩기능있는텍스트필드()
        ...
        // 뷰가 변하면 ====> 뷰 모델이 가지고 있는 데이터가 변하도록 ⭐️⭐️⭐️
        tf.바인딩하기 { [weak self] text in
            self?.viewModel.emailString.데이터값 = text
        }
        /*
         즉 이전시간과 반대로 데이터가 변하면 뷰가 바뀌게 구현한게 아니라
         뷰에서 입력한 즉 뷰가 변하면 데이터가 변하도록 구현한것
         
         사실 1대1 개념임 데이터가 변해도 뷰가 바뀌고, 뷰가 바뀌면 데이터도 바뀌는
         */
        return tf
    }()
    ...
    private lazy var passwordTextField: UITextField = {
        ...
        // 뷰가 변하면 ====> 뷰 모델이 가지고 있는 데이터가 변하도록 ⭐️⭐️⭐️
        tf.바인딩하기 { [weak self] text in
            self?.viewModel.passwordString.데이터값 = text
        }
    ...
    // 뷰 모델이 가지고 있는 데이터가 변하면 ====> 뷰도 변함 ⭐️⭐️⭐️
    func bindViewModel() {
    ...
    // MARK: - 로그인 버튼 눌렸다는 것 뷰모델에 전달 ⭐️⭐️⭐️ (Input)
    // - 1. 로그인 버튼 눌리면 뷰모델에게 처리해달라고 전달
