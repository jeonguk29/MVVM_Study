...
protocol CalculatorType {
    ...
    // 재대로 반환시 BMI 그렇지 안을때는 throws 키워드를 붙여 에러 던질수 있게 해줌
}


// 비즈니스 로직 담당 ⭐️⭐️⭐️
// (BMI계산과 관련된 로직만 담당 - Domain 로직)
// 의존성 주입을 위해 프로토콜을 채택하여 구현
struct BMICalculatorManager: CalculatorType {
    ...
    func calculateBMI(height: String, weight: String) throws -> BMI {
        ...
        guard let h = Double(height), let w = Double(weight) else {
            throw CalculateError.noNumberError // 에러던짐
        }
        
        // 0이상인지 확인
        guard h > 0, w > 0 else {
            throw CalculateError.minusNumberError // 에러던짐
        }
